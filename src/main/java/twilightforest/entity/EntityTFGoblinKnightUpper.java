...
import static com.dunk.tfc.api.Enums.EnumDamageType.SLASHING;

public class EntityTFGoblinKnightUpper extends EntityMob implements IInnateArmor {

	private static final int SHIELD_DAMAGE_THRESHOLD = 500;
	...
	protected void applyEntityAttributes() {
		...
		this.getEntityAttribute(SharedMonsterAttributes.attackDamage).setBaseValue(500.0D); // attack damage
	}
...
//		}
	...
	public boolean checkSlash(EntityPlayer player) {
		if (player.inventory.getCurrentItem().getItem() instanceof ItemWeapon) {
			if (((ItemWeapon) player.inventory.getCurrentItem().getItem()).getDamageType(player) == SLASHING) {
				return true;
			}
		}
		if (player.inventory.getCurrentItem().getItem() instanceof ItemTerraTool) {
			return player.inventory.getCurrentItem().getItem() instanceof ItemCustomScythe;
		}
		if (player.inventory.getCurrentItem().getItem() instanceof ItemCustomAxe) {
			return true;
		}
		return false;
	}
	...
	public boolean attackEntityFrom(DamageSource par1DamageSource, float damageAmount) {
		...
		if (attacker != null) {
			...
			// break armor?
			// Only break if hit with Slashing weapon
			if (attacker instanceof EntityPlayer) {
				EntityPlayer player = (EntityPlayer) par1DamageSource.getEntity();
				if (this.hasArmor() && (difference > 300 || difference < 60) && checkSlash(player)) {
					breakArmor();
				}
			}
		}
	...
	/**
	 * Break our armor
	 * Drop an armor shard when the armor breaks
	 */
